...
public abstract class Region extends AbstractDataModel {
  ...
  protected void add (MemoryCell cell) {
	...
	if (map.lastKey () == (int)cellAddress) {
	  rows.add (cell);
	  tellObservers (new DataModelEvent (DataModelEvent.Type.WRITE_BY_USER, getAllCellsInRow (map.size ())));
	} else
	  rebuildRows ();
  ...
  DataModelEvent update (DataModelEvent event) {
    ...
    /* Handle memory updates from lower addresses upwards. */
    ArrayList<MemoryCell> sortedAffectedCells = new ArrayList<>(affectedCells);
    Collections.sort(sortedAffectedCells, new Comparator<MemoryCell>() {
      @Override
      public int compare(MemoryCell lhs, MemoryCell rhs) {
        return Integer.compare(lhs.getAddress(), rhs.getAddress());
      }
    });

    for (MemoryCell cell : sortedAffectedCells) {
      if (event.getType () == DataModelEvent.Type.WRITE || event.getType () == DataModelEvent.Type.WRITE_BY_USER) {
          if(!map.containsKey(cell.getAddress())) {
              /* cell has been removed (possibly by earlier update) - skip it */
              continue;
          }
