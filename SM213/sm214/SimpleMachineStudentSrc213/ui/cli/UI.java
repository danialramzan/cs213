...
public class UI extends AbstractUI implements Observer, machine.SystemCallInterface {
  
  final static String CLI_USAGE   = "\t[-t <test-filename>] [-ba <benchmark-arch> -bv <benchmark-variant>] \n\t[-r <reg>[:<count>]]* [-m <addr>[:<count>]]* [-c <state>|all] [-p <start-pc>] [-d <dump-filename>] [<file>]";
  ...
  final List <String>  argDumpFilenameList;
  ...
  public UI (ArrayList <String> args) throws ArgException {
    ...
    argDumpFilenameList = getArgList (args, "-d",  false, true,  true, null);
  ...
  public void run () {
    ...
    } else if (argDumpFilenameList .size() > 0) {
      for (String file : argDumpFilenameList)
        cmd .dump (file);
    } else {
  ...
  private class CommandHandler implements CliParser.CommandHandler {
    ...
    public void dump (String filename) {
      try {
        String dumpFilename = filename + ".machine";
        machine .memory .loadFile   (filename);
        machine .memory .dumpToFile (dumpFilename);
      } catch (FileNotFoundException e) {
        System.out.printf ("File %s not found.\n", filename);
      } catch (Memory.FileTypeException fte) {
        System.out.printf ("File %s must end in '.s' for assembly or '.machine' for machine code\n", filename);
    ...
    @Override public void examineProc (String state) {
      boolean match = false;
      String[] ss = state!=null? state.split ("\\.") : null;
      for (RegisterSet rs : machine.processorState)
        if (ss==null || rs.getName().equalsIgnoreCase (ss[0])) {
          if (ss==null)
            System.out.printf ("[%s] CPU State:\n", rs.getName());
          for (Register reg : rs.getAll())
            if (ss!= null && ss.length==2) {
              if (reg.getName().equalsIgnoreCase (ss[1])) {
        ...
        if (! match)
          System.out.printf ("Invalid CPU state name.\n");
    }
    ...
    @Override public void assertProc (String state, int value, String desc) {
      if (assertDesc != null)
        desc = assertDesc + ((desc != null)? (" - " + desc) : "");
      String[] ss = state!=null? state.split ("\\.") : null;
      if (ss != null && ss.length == 2)
        for (RegisterSet rs : machine.processorState)
          if (rs.getName().equalsIgnoreCase (ss[0])) {
            for (Register r : rs.getAll()) {
              if (r.getName().equalsIgnoreCase (ss[1])) {
                if (r.get() != value)
                  System.out.printf ("XXX ASSERTION FAILURE%s: CPU %s == 0x%x != 0x%x\n",
                                     desc!=null? " ("+desc+")" : "", state, r.get(), value);
                return;
              }
            }
          }
    ...
    @Override public int getMemValue (int addr) {
      try {
        return machine .mainMemory .readInteger (addr);
      } catch (Exception e) {
        System.out.printf ("Illegal address 0x%x\n.", addr);
        throw new CommandException();
      }
    }
  }
  ...
  public int handleExecSyscall(byte[] command) {
    ...
    if("1".equals(System.getenv("SIMPLE_MACHINE_ALLOW_EXEC"))) {
      ...
      System.err.println("<<<WOULD EXECUTE " + commandStr + ">>>");
    }
    return 0;
  }
}
