...
public class UI extends AbstractUI {
  ...
  private final static int                  fontSizeDelta       = IS_MS_WINDOWS? 2 : 0;
  private final Font                        addressFont         = new Font  (monoSpaceFont, Font.PLAIN, 10 + fontSizeDelta);
  private final Font                        cellFont            = new Font  (monoSpaceFont, Font.PLAIN, 10 + fontSizeDelta);
  private final Font                        memAddressFont      = new Font  (monoSpaceFont, Font.PLAIN, 9  + fontSizeDelta);
  private final Font                        memCellFont         = new Font  (monoSpaceFont, Font.PLAIN, 9  + fontSizeDelta);
  private final Font                        macFont             = new Font  (monoSpaceFont, Font.PLAIN, 8  + fontSizeDelta);
  private final Font                        labelFont           = new Font  (monoSpaceFont, Font.PLAIN, 10 + fontSizeDelta);
  private final Font                        animationFont       = new Font  (monoSpaceFont, Font.BOLD,  12 + fontSizeDelta);
  private final Font                        titleFont           = new Font  ("Default",     Font.BOLD,  14 + fontSizeDelta);
  private final Font                        subTitleFont        = new Font  ("Default",     Font.BOLD,  12 + fontSizeDelta);
  private final Font                        aboutFont           = new Font  ("Default",     Font.PLAIN, 10 + fontSizeDelta);
  private final Font                        nameFont            = new Font  ("Default",     Font.PLAIN, 10 + fontSizeDelta);
  private final Font                        commentFont         = new Font  ("Arial",       Font.PLAIN, 10 + fontSizeDelta);
  private final Font                        statusMessageFont   = new Font  ("Default",     Font.PLAIN, 11 + fontSizeDelta);
  private final Font                        curInsFont          = new Font  ("Default",     Font.PLAIN, 20 + fontSizeDelta);
  private final Font                        curInsDscFont       = new Font  ("Default",     Font.PLAIN, 11 + fontSizeDelta);
  private final Font                        curInsTwoColFont    = new Font  ("Default",     Font.PLAIN, 13 + fontSizeDelta);
  private final Font                        curInsDscTwoColFont = new Font  ("Default",     Font.PLAIN, 9  + fontSizeDelta);
  ...
  final static Font                   TITLE_FONT         = new Font ("Default", Font.BOLD, 12+fontSizeDelta);
  ...
  class Application extends JFrame implements Observer, machine.SystemCallInterface {
    AboutBox                   aboutBox;
    ImageIcon                  icon=null;
    MainPane                   mainPane;
    StatusBar                  statusBar;
    ...
    Application () {
      ...
      machine.setSyscallInterface(this);
	...
	try {
	  ...
	  OSXAdapter.setQuitHandler  (this, getClass ().getDeclaredMethod ("quit",      (Class[]) null));
          if (icon != null) {
            OSXAdapter.setDockIcon (icon.getImage());
          }
          OSXAdapter.enableFullScreen (this);
	} catch (Exception e) {
	...
	for (RegisterSet regSet : machine.processorState) {
	  ...
	  if (isTwoProcStateCols) 
	    pane = new ViewPane ("", 
				 ...
				 Arrays.asList  (new ViewLabel     (0, -3, SwingConstants.RIGHT, nameFont, nameColor, null,  new ViewFormat.Format (String.class, "%s:")),
						 ...
						 new ViewLabel     (0, -3, SwingConstants.RIGHT, nameFont, nameColor, null,  new ViewFormat.Format (String.class, "%s:")),
						 new ViewTextField (8,  SwingConstants.LEFT,   cellFont, null,      null, 
								    Arrays.asList ((ViewFormat.Format) 
										   new ViewFormat.NumberFormat     (Integer.class,        "%08x", 16),
										   new ViewFormat.NumberFormat     (Long.class,           "%d",   10),
	    ...
	    pane = new ViewPane (curInsAddr!=null? "" : regSet.getName (), 
				 ...
				 Arrays.asList  (new ViewLabel     (0, -10, SwingConstants.RIGHT, nameFont, nameColor, null,  new ViewFormat.Format (String.class, "%s:")),
	...
	super ("Register File", 
	       ...
	       Arrays.asList (new ViewLabel     (0, -4, SwingConstants.RIGHT,  addressFont, addressColor, MEMORY_HIGHLIGHT, new ViewFormat.Format       (String.class, "%s:")),
	...
	super (name, 
	       ...
	       Arrays.asList ((ViewFormat) 
			      new ViewLabel    (-width, 0, SwingConstants.CENTER,  isTwoProcStateCols? curInsTwoColFont : curInsFont, codeColor, null, new ViewFormat.Format (String.class, "%s"))));
	...
	super ("", 
	       ...
	       Arrays.asList ((ViewFormat) 
			      new ViewLabel    (-width, 0, SwingConstants.CENTER, isTwoProcStateCols? curInsDscTwoColFont : curInsDscFont, macColor, null, new ViewFormat.Format (String.class, "%s"))));
	...
	super (String.format ("Memory - %x", address),
	       ...
	       Arrays.asList (new ViewLabel     (0, 8, SwingConstants.RIGHT,  memAddressFont, addressColor, MEMORY_HIGHLIGHT, new ViewFormat.NumberFormat (Integer.class, "0x%x:", 16)),
			      ...
			      new ViewTextField (2,  SwingConstants.CENTER, memCellFont,    null,         MEMORY_HIGHLIGHT, new ViewFormat.NumberFormat (Byte.class,    "%02x", 16)),
			      new ViewTextField (2,  SwingConstants.CENTER, memCellFont,    null,         MEMORY_HIGHLIGHT, new ViewFormat.NumberFormat (Byte.class,    "%02x", 16))) :
	       Arrays.asList (new ViewLabel     (0, 8, SwingConstants.RIGHT,  addressFont,    addressColor, MEMORY_HIGHLIGHT, new ViewFormat.NumberFormat (Integer.class, "0x%x:", 16)),
	...
	super (String.format ("Instructions - %x", aRegion.getAddress ()),
	       ...
	       Arrays.asList (new ViewCheckBox  (breakpointColor),
	 		      new ViewLabel     (0, 8, SwingConstants.RIGHT, addressFont, addressColor, CODE_HIGHLIGHT, new ViewFormat.NumberFormat (Integer.class, "0x%x:", 16)),
			      ...
			      new ViewTextField (-14,SwingConstants.LEFT,  commentFont, commentColor, CODE_HIGHLIGHT, new ViewFormat.Format       (String.class,  "%s")))
               :
	       Arrays.asList (new ViewCheckBox  (breakpointColor),
			      new ViewLabel     (0, 8, SwingConstants.RIGHT, addressFont, addressColor, CODE_HIGHLIGHT, new ViewFormat.NumberFormat (Integer.class, "0x%x:", 16)),
			      new ViewTextField (8,  SwingConstants.RIGHT, labelFont,   labelColor,   CODE_HIGHLIGHT, new ViewFormat.Format       (String.class,  "%s")),
	...
	for (Region r : machine.memory.getRegions ()) {
	  ...
	  if (r.getType () == Region.Type.INSTRUCTIONS) {
        ...
        int x      = Application.this.getX();
        int y      = Application.this.getY();
        int width  = Math.max (getPreferredSize().width, getSize().width);
        int height  = Math.max (getPreferredSize().height, getSize().height);
    ...
    /* TODO: This should probably live in a utility class somewhere */
    byte[] hexToBytes(String s) throws IllegalArgumentException {
      ...
      if(optr % 2 == 1) {
        throw new IllegalArgumentException("Input has an odd number of hex digits");
      }
      return Arrays.copyOf(result, optr / 2);
    }
    
    /* We don't want to have any crashes due to charset conversion, which is why we decode as ISO-8859-1 and encode as UTF-8.
    In practice this means that non-ASCII characters are gonna be borked. Use the CLI if you want proper behaviour.
     */
    boolean hexInput = false;
    @Override
    public byte[] handleReadSyscall(int fd, int size) {
      statusBar.showMessage ("Waiting for user input...");
      Object[] options = { "Send and Continue", "Send and Halt" };

      JPanel panel = new JPanel();
      JTextField textField = new JTextField(32);
      JCheckBox hexCheck = new JCheckBox("Hex Input", hexInput);
      panel.add(textField);
      panel.add(hexCheck);
      ...
      while(true) {
        ...
        if(result == CLOSED_OPTION) {
          /* This eventually returns -1 to the syscall. */
          data = null;
          break;
        }
        if(hexCheck.isSelected()) {
          hexInput = true;
          try {
            data = hexToBytes(textField.getText());
          ...
          hexInput = false;
          data = textField.getText().getBytes(Charset.forName("UTF-8"));
        }
        break;
      }
      if(result != YES_OPTION) {
        /* User chose to halt or cancel dialog */
        machine.stop();
        animator.stop();
      }
      return data;
    ...
    @Override
    public int handleWriteSyscall(int fd, byte[] data) {
      statusBar.showMessage ("Displaying output to user...");
      Object[] options = {"Continue", "Halt"};
    ...
    @Override
    public int handleExecSyscall(byte[] command) {
      String commandStr = new String(command, Charset.forName("ISO-8859-1"));
      statusBar.showMessage (String.format("Executing command %s...", commandStr));
      if("1".equals(System.getenv("SIMPLE_MACHINE_ALLOW_EXEC"))) {
        /* Danger zone! */
        try {
          if(System.getProperty("os.name").toLowerCase().contains("win")) {
            new ProcessBuilder().command("cmd.exe", "/c", commandStr).inheritIO().start();
          } else {
            new ProcessBuilder().command("/bin/sh", "-c", commandStr).inheritIO().start();
          }
        } catch(IOException e) {
          e.printStackTrace();
          JOptionPane.showMessageDialog(this, "Failed to execute command \"" + commandStr + "\": " + e);
          return -1;
        }
      } else {
        JOptionPane.showMessageDialog(this, "Would execute command \"" + commandStr + "\"");
      }
      return 0;
    }
  }
