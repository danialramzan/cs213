...
import machine.SystemCallInterface;
...
public class Machine extends Observable implements Observer {
  ...
  private SystemCallInterface      syscallInterface;
  ...
  public String run (boolean isSingleStep, int aMilliseconds) {
    try {
      ...
      while(true) {
        try {
          cpu.start();
        } catch(SystemCallInterface.SyscallInterrupt syscall) {
          int result = handleSyscall(syscall);
          cpu.setSyscallReturn(result);
          if(!cpu.isInterrupt())
            continue;
        }
        break;
  ...
  public void setSyscallInterface(SystemCallInterface sci) {
    syscallInterface = sci;
  }

  private int handleSyscall(SystemCallInterface.SyscallInterrupt syscall) throws UserVisibleException {
    if(syscallInterface == null)
      return -1;
    switch(syscall.nr) {
      case 0: // read(fd, buf, size)
      {
        int maxlen = syscall.args[2];
        if(maxlen < 0)
          return -1;
        else if(maxlen == 0)
          return 0;

        byte[] data = syscallInterface.handleReadSyscall(syscall.args[0], maxlen);
        if(data == null)
          return -1;
        ...
        if(data.length > maxlen) {
          /* XXX This should probably be an error: extra data is simply discarded */
          data = Arrays.copyOf(data, maxlen);
        }
        mainMemory.writeUnaligned(syscall.args[1], UnsignedByte.toUnsignedBytes(data));
        return data.length;
      }
      case 1: // write(fd, buf, size)
      {
        int wlen = syscall.args[2];
        if(wlen < 0)
          return -1;
        else if(wlen == 0)
          return 0;
        
        UnsignedByte[] data = mainMemory.readUnaligned(syscall.args[1], wlen);
        return syscallInterface.handleWriteSyscall(syscall.args[0], UnsignedByte.toBytes(data));
      }
      case 2: // exec(cmdstr, size)
      {
        int clen = syscall.args[1];
        if(clen < 0)
          return -1;

        UnsignedByte[] data;
        if(clen == 0) {
          data = new UnsignedByte[0];
        } else {
          data = mainMemory.readUnaligned(syscall.args[0], clen);
        }
        return syscallInterface.handleExecSyscall(UnsignedByte.toBytes(data));
      }
      default:
        throw cpu.new InvalidInstructionException();
    }
  }
      ...
      try {
        ...
        if (o==cpu) {
          ...
          if (!isFirstInstruction && isSingleStepEnabled) {
            ...
            try {Thread.sleep (pauseMilliseconds);} catch (InterruptedException e) {}
            pausedThread = null;
          }
