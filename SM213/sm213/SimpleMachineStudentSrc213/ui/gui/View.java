...
class View extends JTable implements ViewModel.Listener, UndoableEditListener {
  ...
  private int getColumnHeaderWidth (int columnNumber) {
    String header = (String) columnModel.getColumn (columnNumber) .getHeaderValue ();
    javax.swing.table.TableCellRenderer tr = columnModel.getColumn (columnNumber) .getHeaderRenderer();
    if (tr == null)
      tr = getTableHeader().getDefaultRenderer();
    Component rc = tr.getTableCellRendererComponent (this, header, false, false, 0, columnNumber);
    int width = rc.getFontMetrics (rc.getFont()) .stringWidth (header);
    // XXX For some reason you need to pad this
    width += 10;
    return width;
  }
  ...
  private void setupColumns () {
    ...
    for (int i=0; i<model.getColumnCount (); i++) {
      ...
      column.setPreferredWidth (Math.max (columnFormat.getWidth (), getColumnHeaderWidth (i)));
      if (!columnFormat.isWidthVariable()) {
        column.setMinWidth (Math.max (columnFormat.getWidth (), getColumnHeaderWidth (i)));
        column.setMaxWidth (Math.max (columnFormat.getWidth (), getColumnHeaderWidth (i)));
      }
