...
public class Memory extends AbstractDataModel implements Observer {
  ...
  public void dumpToFile (String filename) throws FileNotFoundException {
    if (filename != null) {
      ...
      for (Region r : regions) {
        for (MemoryCell c : r .getSavableRows()) {
          ps .print (String .format ("%x: ", c .getAddress()));
          for (Byte b : c .getValue() .toBytes())
  ...
  /**
   * Exception for errors in input file type.
   */
  public static class FileTypeException extends Exception {}
  
  /**
   * Load a file into memory where type of file is determined by it's extension (i.e., *.s for assembly langauge and *.machine for machine code).
   *
